...
arme_bmw(ListaArme) :- findall(Arma, (inarmat(Nume, Arma), conduce(Nume, bmw)), ListaArme).
...
arme_marca(Marca, ListaArme) :- bagof(Arma, Nume^(inarmat(Nume, Arma), conduce(Nume, Marca)), ListaArme).
...
helper([]).
helper([A|B]) :- au_arma(A, ListaNume), length(ListaNume, N), N > 1, helper(B).

suspect1(Nume:Marca:Arma) :- suspect(Nume:Marca:Arma), arme_marca(Marca, ListaArme), helper(ListaArme).
