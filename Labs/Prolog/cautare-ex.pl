...
next_state(misionari, state(est, NM_Est_initial, NC_Est_initial, NM_Vest_initial, NC_Vest_initial), state(vest, NM_Est_final, NC_Est_final, NM_Vest_final, NC_Vest_final)):-
        boat(M, C),
        NM_Est_initial >= M,
        NM_Est_final is NM_Est_initial - M,
        NC_Est_final is NC_Est_initial - C,

        NC_Est_initial >= C,
        NM_Vest_final is NM_Vest_initial + M,
        NC_Vest_final is NC_Vest_initial + C,

        safeMisionari(NM_Vest_final, NC_Vest_final),
        safeMisionari(NM_Est_final, NC_Est_final).

next_state(misionari, state(vest, NM_Est_initial, NC_Est_initial, NM_Vest_initial, NC_Vest_initial), state(est, NM_Est_final, NC_Est_final, NM_Vest_final, NC_Vest_final)):-
        boat(M, C),
        NM_Vest_initial >= M,
        NC_Vest_initial >= C,
...
preorder(N, Parc) :- nod(N), \+ arc(N, _), Parc = [N].
preorder(N, Parc) :- nod(N), arc(N, Chld), cDFS(Chld, [], PChld), Parc = [N | PChld].

cDFS([], Visited, Parc) :- reverse(Visited, Parc).
cDFS([R | Children], Visited, Parc) :- nod(R), \+ arc(R, _), cDFS(Children, [R | Visited], Parc).
cDFS([R | Children], Visited, Parc) :- nod(R), arc(R, CChld), cDFS(CChld, [R | Visited], PChild),
                              reverse(PChild, VisitedChild),
                               cDFS(Children, VisitedChild, Parc).
