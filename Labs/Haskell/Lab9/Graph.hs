...
dfs g start = traversal [(start, Nothing)] []
    where 
        traversal [] vizited = vizited
        traversal ((node, parent) : arcFound) vizited 
            | elem node (map (fst) vizited) = traversal arcFound vizited
            | otherwise = traversal ([(a, Just node) | a <- nodeOut g node] ++ arcFound) (vizited ++ [(node, parent)])

{-
    6. (2.5p) Implementați algoritmul de parcurgere în lățime. (Breadth-First Search)
-}
bfs :: Graph g => TraverseAlgo g
bfs g start = traversal [(start, Nothing)] []
    where   
        traversal [] vizited = vizited
        traversal ((node, parent) : arcsFound) vizited
            |elem node (map fst vizited) = traversal arcsFound vizited
            |otherwise = traversal (arcsFound ++ [(a, Just node) | a <- nodeOut g node ]) (vizited ++ [(node, parent)])
