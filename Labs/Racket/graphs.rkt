...
(define (outgoing G v)
    (map cadr (filter (λ (x) (equal? (car x) v)) (get-edges G))))
...
;; o veți implementa obligatoriu folosind funcționale și/sau funcții anonime, fără recursivitate
(define (incoming G v) 
    (map car (filter (λ (x) (equal? (cadr x) v)) (get-edges G))))
...
(define (dfs-one G v)
  (let dfsValuaaare ((todo (map (λ(x) (cons x v)) (outgoing G v))) (facutu-s-a (list (cons v 'none)))) 
    (if (null? todo)
        facutu-s-a
        (if (assoc (caar todo) facutu-s-a)
            (dfsValuaaare (cdr todo) facutu-s-a)
            (dfsValuaaare (append (map (lambda(x) (cons x (caar todo))) (outgoing G (caar todo))) todo) (append facutu-s-a (list (car todo))) )))))
...
(define (dfs G)
  (let dfs-all ((nodes-to-visit (get-nodes G)) (aux '()) (result '()))
    (if (null? nodes-to-visit)
        result
        (if (assoc (car nodes-to-visit) aux)
            (dfs-all (cdr nodes-to-visit) aux result)
            (dfs-all (cdr nodes-to-visit) (append aux (dfs-two G (car nodes-to-visit) aux)) (append result (list (dfs-two G (car nodes-to-visit) aux)))))))) 


(define (dfs-two G v all-temp)
  (let dfs-aux ((v v) (S (list (cons v 'none))))
    (foldl (λ (w s)
             (if (and (not (assoc w s)) (not (assoc w all-temp)))
                 (dfs-aux w (append s (list (cons w v))))
                 s))
          S (outgoing G v))))
