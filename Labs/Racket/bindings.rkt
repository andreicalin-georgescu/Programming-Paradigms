...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((seg (get-line-segment)) (st (get-start-point seg)) (end (get-end-point seg)) (diff-x (- (car end) (car st)))
                                  (diff-y (- (cdr end) (cdr st))) (diff-x-sq (* diff-x diff-x)) (diff-y-sq (* diff-y diff-y)))
    (sqrt (+ diff-x-sq diff-y-sq))))
...
(define (distance p1 p2)
  (let (( seg (λ () (cons p1 p2))) (st car) (end cdr))
    (compute-length seg st end)))
...
(define (compute-perimeter points)
  (let my-loop ((p points) (result 0))
    (if (equal? 1 (length p))
        (+ result (distance (car p) (car points)))
        (my-loop (cdr p) (+ result (distance (car p) (cadr p)))))))
...
(define (compute-sum L)
  (if (null? L)
      0
      (+ (car L) (compute-sum (cdr L)))))

(define (3-sequence-max numbers separator)
  (define (my-separator x)
    (not (equal? x separator)))
  (let*-values
      ([(l1 l2) (splitf-at numbers my-separator)]
       [(l2 l3) (splitf-at (cdr l2) my-separator)])
    (let* ((sum-l1 (compute-sum l1)) (sum-l2 (compute-sum l2)) (sum-l3 (compute-sum l3)))
...
(define (list-num-concat numbers)
  (string->number (foldr (λ (x acc) (string-append (number->string x) acc)) "" numbers)))
;;map pe fiecare arg si apply pe fiecare nr
...
(define (find-all-suffixes number)
  (let my_tag ((L (number->list number)) (acc '()))
    (if (null? L)
        acc
        (my_tag (cdr L) (append acc (list (list-num-concat L)))))))
...
(define (run initial-state final-state next)
  (let label ((current initial-state) (acc '()))
    (if (equal? current final-state)
        (append acc (list current))
        (label (next current) (append acc (list current))))))
...
(define (generate-number k x)
  (let* ((init x) (func (λ (i) (+ i k))) (fin (+ init (* k (sub1 k)))))
    (list-num-concat (run init fin func))))
